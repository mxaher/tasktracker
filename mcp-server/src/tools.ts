import { CloudflareClient, type Worker, type WorkerContent, type D1Database, type D1Table } from './cloudflare.js';

export function createWorkerTools(cf: CloudflareClient) {
  return [
    {
    ...
    },
    {
      ...
      async execute(args: { scriptName: string }) {
      ...
      async execute(args: { scriptName: string }) {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Worker "${args.scriptName}" deleted successfully`,
            },
          ],
        };
      },
    ...
    },
    {
      ...
      async execute(args: { scriptName: string; script: string }) {
        await cf.uploadWorker(args.scriptName, args.script);
        return {
          content: [
            {
              type: 'text' as const,
              text: `Worker "${args.scriptName}" uploaded successfully`,
            },
          ],
        };
      },
...
export function createD1Tools(cf: CloudflareClient) {
  return [
    {
    ...
    },
    {
      ...
      async execute(args: { databaseId: string }) {
    ...
    },
    {
      ...
      async execute(args: { databaseId: string; query: string; params?: string[] }) {
    ...
    },
    {
      ...
      async execute(args: { databaseId: string; queries: Array<{ sql: string; params?: string[] }> }) {
    ...
    },
    {
      ...
      async execute(args: { databaseId: string; tableName: string }) {
